    ...
    return (
       <>
            {posts.length === 0 ? (
                ...
                posts.map((post) => (
                    <div onClick={() => handlePostClick(post.id)} key={post.id} className="w-full rounded-lg bg-white dark:bg-[#1e293b] overflow-hidden shadow hover:scale-102 hover:shadow-primary/25 duration-300 cursor-pointer">
                        <img alt="" className="aspect-video" src={`${post.directory}/${post.filename}`}/>
                        <span className="ml-5 mt-4 px-3 py-1 inline-block bg-primary/20 rounded-full text-primary text-xs">{titleCase(post.category.name)}</span>
                        <div className="p-5">
